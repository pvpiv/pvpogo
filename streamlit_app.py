...
# Helper
    ...
    box = text_input(label=today.strftime("%m/%d/%y"),value=str(location['latitude']) + ',' + str(location['longitude']))
    box = placeholder.empty()
    box = text_input(label=today.strftime("%m/%d/%y"), value='*Click string to show Copy button and Paste Top ' + topstrin + ' into PokeGO*', label_visibility='hidden', disabled=True, key="sstring")
